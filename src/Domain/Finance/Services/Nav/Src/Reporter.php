...
class Reporter
{
     ...
     *
     * @param  array  $invoiceNumberQuery
     *                                     $invoiceNumberQuery: Az invoiceNumberQuery-nek megfelelően összeállított lekérdezési adatok
     * @param  bool  $returnDecodedInvoiceData
     *                                          $returnDecodedInvoiceData: invoiceDataResult helyett a dekódolt számla XML-t adja vissza a metódus
    ...
    public function queryInvoiceData($invoiceNumberQuery, $returnDecodedInvoiceData = false)
    {
        ...
        if ($returnDecodedInvoiceData) {
            ...
            $isCompressed = false;
            if (method_exists($result, 'compressedContentIndicator')) {
     ...
     *
     * $invoiceQueryParams: Az invoiceQueryParams-nak megfelelően összeállított lekérdezési adatok
     * $page: Oldalszám (1-től kezdve a számozást)
     * $direction A keresés iránya, a keresés elvégezhető kiállítóként és vevőként is [OUTBOUND, INBOUND]
     * return: SimpleXMLElement $queryResultsXml A válasz XML invoiceDigestResult része
     */
    public function queryInvoiceDigest(array $invoiceQueryParams, int $page = 1, string $direction = 'OUTBOUND'): SimpleXMLElement
    {
     ...
     */
    public function queryTransactionStatus(string $transactionId, bool $returnOriginalRequest = false): SimpleXMLElement
    {
     ...
     */
    public function queryInvoiceChainDigest(array $invoiceChainQuery, int $page = 1): SimpleXMLElement
    {
     ...
     *
     * $taxNumber: Adószám, pattern: [0-9]{8}
     ...
     */
    public function queryTaxpayer(string $taxNumber): bool|SimpleXMLElement|null
    {
     ...
     * return: Token
     */
    public function tokenExchange(): string
    {
     ...
     */
    public static function getInvoiceValidationError(SimpleXMLElement $xml): ?string
    {
        try {
