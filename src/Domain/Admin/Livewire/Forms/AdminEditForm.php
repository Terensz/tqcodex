 ...
 */
class AdminEditForm extends BaseLivewireForm implements EditFormInterface
{
    /**
    ...
    public function getModelRules()
    {
        ...
        $rules = [];
        $rulesBase = AdminRules::rules($this->admin->id);
    ...
    public static function getFormData(?Model $model = null, ?string $actionType = null): array
    {
        return [
            [
                BaseLivewireForm::FORM_DATA_KEY_PROPERTY => Admin::PROPERTY_LAST_NAME,
            ...
            [
                BaseLivewireForm::FORM_DATA_KEY_PROPERTY => Admin::PROPERTY_FIRST_NAME,
                BaseLivewireForm::FORM_DATA_KEY_INPUT_TYPE => ValidationHelper::INPUT_TYPE_TEXT,
                BaseLivewireForm::FORM_DATA_KEY_TRANSLATION_REFERENCE => 'user.Firstname',
            ],
            [
                BaseLivewireForm::FORM_DATA_KEY_PROPERTY => Admin::PROPERTY_EMAIL,
                BaseLivewireForm::FORM_DATA_KEY_INPUT_TYPE => ValidationHelper::INPUT_TYPE_TEXT,
    ...
    {
        $rolesOfAdmin = $this->admin->roles()->get();
    ...
    public function store()
    {
        ...
        foreach (self::getFormData() as $formDataRow) {
            ...
            if (! defined('self::TECHNICAL_PROPERTIES') || (defined('self::TECHNICAL_PROPERTIES') && ! in_array($property, self::TECHNICAL_PROPERTIES))) {
                $this->admin->{$property} = $this->{$property};
                $properties[] = $property;
            }
        }
        ...
        if (! empty($this->technicalPassword)) {
            $this->admin->password = Hash::make($this->technicalPassword);
        }

        /**
         * This property varies!
         */
        $this->admin->saveWithRoleData($this->role_data);
    }
