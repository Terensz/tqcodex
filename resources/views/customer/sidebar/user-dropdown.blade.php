...
<!-- <li>
  ...
  <ul role="list" class="-mx-2 mt-2 space-y-1">
    ...
    <li>
      <a href="{{route('customer.dashboard', ['access_token' => AccessTokenService::getAccessToken(UserService::ROLE_TYPE_CUSTOMER)]) }}" class="text-gray-400 hover:text-white hover:bg-gray-800 group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
        ...
        <div @click.away="open = false" class="relative" x-data="{ open: false }">
          <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="bottom-0 origin-top-right absolute left-0 mt-2 -mr-1 w-48 rounded-md shadow-lg">
            <div class="py-1 rounded-md bg-white shadow-xs relative">
              <div class="mt-2">
                <a href="{{ route('customer.profile.edit', ['access_token' => AccessTokenService::getAccessToken(UserService::ROLE_TYPE_CUSTOMER)]) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150">{{ __('user.Profile') }}</a>
              </div>
              <div class="mb-3">
          ...
          <div>
            <button @click="open = !open" class="max-w-xs flex items-center text-sm rounded-full text-white focus:outline-none focus:shadow-solid transition ease-in-out duration-150">
              @if (UserService::getUser(UserService::ROLE_TYPE_CUSTOMER)->photo_path)
              <img class="inline-block h-10 w-10 rounded-full" src="{{ UserService::getUser(UserService::ROLE_TYPE_CUSTOMER)->photo_path }}" alt="{{ UserService::getUser(UserService::ROLE_TYPE_CUSTOMER)->name }}">
              @else
              <!-- <img class="inline-block h-10 w-10 rounded-full" src="/images/Shared/man-avatar.svg" alt="{{ UserService::getUser(UserService::ROLE_TYPE_CUSTOMER)->name ?? 'Felhasználó' }}"> -->
              @endif
              <div class="ml-3">
                <p class="text-base leading-6 font-medium text-blue-400 group-hover:text-blue-700">
                  {{ UserService::getUser(UserService::ROLE_TYPE_CUSTOMER)->name ?? 'Felhasználó' }}
                </p>
