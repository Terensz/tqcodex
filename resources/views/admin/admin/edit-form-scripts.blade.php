@php 
use Domain\User\Services\PermissionService;
use Domain\Shared\Livewire\Base\BaseEditComponent;
@endphp
@script
<script>
    $wire.on('check-all-permission-in-guard', () => {
        $wire.$call('checkAllPermissionInGuard');
    });
    $wire.on('refresh-view', () => {
        $wire.$call('refreshView');
    });
    $wire.on('check-all-permission-in-suffix', (suffix) => {
        $wire.$call('checkAllPermissionInSuffix', suffix);
    });
    $wire.on('refresh-guard-permissions', () => {
        $wire.$call('refreshGuardPermissions');
    });
    $wire.on('save-successful', (params) => {
        console.log('params: ', params);
        let actionType = params[0].actionType;
        let id = params[0].id;
        if (actionType == '{{ BaseEditComponent::ACTION_TYPE_NEW }}' && !isNaN(id) && parseInt(id) > 0) {
            window.location.href = params[0].editRoute;
        }
        Toaster.success("{{ __('shared.SaveSuccessful') }}");
    });
    $wire.on('save-failed', () => {
        Toaster.error("{{ __('shared.SaveFailed') }}");
    });
</script>
@endscript