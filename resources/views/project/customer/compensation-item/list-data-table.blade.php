                    ...
                    <tr>
                        ...
                        <th class="border-b dark:border-slate-600 font-medium p-4 pl-8 pt-0 pb-3 text-slate-400 dark:text-slate-200 text-left" style="cursor: pointer;">
                            @if($fieldData[BaseListComponent::INPUT_TYPE] === 'text')
                                <input wire:model="{{ $fieldData[BaseListComponent::PROPERTY_ALIAS] }}" 
                                    class="dataGrid-{{ $fieldData[BaseListComponent::PROPERTY_ALIAS] }} {{ $textInputClass }}" 
                            ...
                            @elseif($fieldData[BaseListComponent::INPUT_TYPE] === 'select2')
                                <div wire:ignore>
                                    <select id="inputImitation_{{ $fieldData[BaseListComponent::PROPERTY_ALIAS] }}" multiple class="multiselect-input form-control dataGrid-{{ $fieldData[BaseListComponent::PROPERTY_ALIAS] }}" autocomplete="false">
                            ...
                            <div wire:click="doSort('{{ $fieldData[BaseListComponent::PROPERTY_ALIAS] }}')" style="width: 100%; margin-top: 10px; display: flex; align-items: center;">
                                <span>{{ __($fieldData[BaseListComponent::TRANSLATION_REFERENCE]) }}</span>
                                @if ($sortColumn == $fieldData[BaseListComponent::PROPERTY_ALIAS])
                                    @if ($sortDirection == 'ASC')
                                    <img wire:click="toggleSortDirection('{{ $fieldData[BaseListComponent::PROPERTY_ALIAS] }}')" src="/Bootstrap-icons/chevron-down.svg">
                                    @else
                                    <img wire:click="toggleSortDirection('{{ $fieldData[BaseListComponent::PROPERTY_ALIAS] }}')" src="/Bootstrap-icons/chevron-up.svg">
                                    @endif
                                @else
                    ...
                    <tr class="DataGrid-row">
                        @foreach($tableData as $fieldData)
                            ...
                            <td class="dataTable-td border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                ...
                                // dump($object);
                                /**
                                 * Displaying cell value
                                */
                                $contactProfile = $object->contact()->first()->getContactProfile();
                                $recorderOrg = $object->recorderOrg()->first();
                                $propertyAlias = $fieldData[BaseListComponent::PROPERTY_ALIAS];
                                if ($fieldData[BaseListComponent::TABLE_ALIAS] == 'compensationitems') {
                                    $value = $object->$propertyAlias;
                                }
                                if ($fieldData[BaseListComponent::TABLE_ALIAS] == 'contactprofiles') {
                                    if ($propertyAlias == 'contact_lastname') {
                                        $value = $contactProfile->lastname;
                                    }
                                    if ($propertyAlias == 'contact_firstname') {
                                        $value = $contactProfile->firstname;
                                    }
                                }
                                if ($fieldData[BaseListComponent::TABLE_ALIAS] == 'creatororgs') {
                                    if ($propertyAlias == 'creator_organization_name') {
                                        $value = $recorderOrg->name;
                                    }
                                }
                                ?>
